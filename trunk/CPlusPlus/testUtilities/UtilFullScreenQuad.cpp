...
void UtilFullScreenQuad::Init( iDevice* pDevice, unsigned int screenWidth, unsigned int screenHeight, int test )
{
	if( pDevice->m_Dx9  != 0 )
	{
		   // Define DEBUG_VS and/or DEBUG_PS to debug vertex and/or pixel shaders with the 
		...
		if (test < 2)
		{
		pDevice->m_Dx9->CreateVertexBuffer(sizeof (Vertices) ,0 ,D3DFVF_CUSTOMVERTEX,D3DPOOL_DEFAULT,&m_pVB9,NULL);
		...
		{
		pDevice->m_Dx9->CreateVertexBuffer(sizeof (VerticesX) ,0 ,D3DFVF_CUSTOMVERTEX,D3DPOOL_DEFAULT,&m_pVB9,NULL);
		VOID * vertArray;
		m_pVB9->Lock (0, sizeof (VerticesX), (void**)&vertArray, 0);
		memcpy(vertArray, VerticesX, sizeof (VerticesX));
		m_pVB9->Unlock();
		}
	}
...
void UtilFullScreenQuad::RenderFullScreenQuad( iDevice* pDevice, int test, int test2, int test3)
{
	if( pDevice->m_Dx9 )
	{
		D3DVIEWPORT9 vp;
...
void UtilFullScreenQuad::RenderFullScreenQuadScissor( iDevice* pDevice, int test, int test2, int test3)
{
	if( pDevice->m_Dx9 )
	{
		D3DVIEWPORT9 vp;
